...
function handleUpdateProduct(id) {
        ...
        <form autocomplete="off" class="add-pool-form w-[1000px] p-5 bg-[#F8F8F8] rounded-[20px]">
            ...
            <div class="flex justify-between mt-10">
                ...
                <div class="w-[49%] flex flex-col gap-[30px]">
                    ...
                    <button class="add-btn bg-[#009398] text-white py-[10px] rounded-[30px] mt-[28px] font-bold text-[22px] hover:opacity-[70%] duration-300 px-[14px]">Обнавить</button>
                </div>
            </div>
        </form>
    `
    ...
    elUpdatePoolForm.addEventListener("submit", function (evt) {
        ...
        `
        setTimeout(() => {
            elSubmitBtn.innerHTML = `Обнавить`
            setTimeout(() => {
                modalWrapper.classList.add("scale-0")
